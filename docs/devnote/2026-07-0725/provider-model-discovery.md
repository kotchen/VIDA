...
- 进一步排查确认，失败不只来自启动环境：原 SSRF 代理生成的 Basic Auth 密码为空，
  `yt-dlp` 不会稳定发送对应的 `Proxy-Authorization`，代理因此返回 403。
- SSRF 代理现为每次会话生成随机、非空用户名和密码，并使用常量时间比较校验完整凭据；平台下载
  仍只能通过受控代理访问外部网络，没有放开 `yt-dlp` 的直连能力。
- v2 的平台下载改为复用 v1 的 Python `yt_dlp.YoutubeDL` 调用方式，由 extractor 完成页面解析、
  分离音视频格式选择、请求头和 Referer 处理，并直接下载单个 `bestaudio/best` 文件。
- 下载适配器增加固定输出模板、文件大小和超时限制、取消检查、异常脱敏、输出目录边界检查，以及
  对多文件、临时文件、符号链接和不安全扩展名的拒绝与清理。
- 启动时显式禁用第三方 `yt-dlp` plugin 搜索，避免运行环境中的外部 plugin 改变 extractor
  行为或绕过预期边界。
- 使用真实 Bilibili 页面 `BV1fp4y1X7JJ` 进行隔离集成验证，成功经 SSRF 代理获取单个
  `.m4a` 音频文件（6,826,647 字节）；随后补充 `noprogress` 配置，避免下载进度污染服务日志。
- 修复分支完整后端验证为 350 个测试通过、1 个跳过；沙箱内禁止 loopback 端口绑定，因此完整验证
  在允许绑定本机 `127.0.0.1` 临时端口的受控环境执行。
...
Basic Auth 协议允许空密码，不代表每个客户端都会为 `username:` 生成认证头。代理鉴权测试不能只用
手写 HTTP 请求，还应验证真实调用方是否发送相同字节。使用随机非空用户名和密码既能保持会话隔离，
也避免了客户端对空密码的差异处理；服务端仍需精确校验用户名和密码，不能只检查“认证头非空”。

### 2.7 平台 extractor 应负责格式与站点语义

Bilibili 等站点常返回分离的音视频格式，顶层元数据不一定存在可直接下载的 `url`。先让 extractor
返回元数据、再由业务代码猜测媒体 URL，会重复实现签名、Referer、format selection 等站点逻辑。
更稳妥的边界是让 `YoutubeDL` 完成受限下载，而业务层负责网络出口、资源上限、取消、输出校验和
错误脱敏。

### 2.8 禁用 plugin 不能只依赖命令行参数

从子进程 CLI 改为 Python API 后，原来的命令行 `--no-plugins` 不再存在于调用链。安全配置应在
构造 downloader 前显式清空 `yt_dlp` 的 plugin 目录，并用 bootstrap 测试锁定初始化顺序，避免
未来重构时在安全边界建立前加载环境 plugin。
...
安全平台下载修复的功能提交范围为 `6c18b77^..1f9ca59`。如果需要恢复修复前行为，在共享分支上
创建反向提交：

```bash
git switch main
git pull --ff-only
git revert --no-commit 6c18b77^..1f9ca59
git commit -m "revert: restore previous v2 yt-dlp ingest"
```

该范围会恢复旧的空密码代理凭据、`yt-dlp` 子进程元数据解析和业务层媒体 URL 下载方式，因此会重新
引入本次确认的 Bilibili 403 与分离格式兼容问题，只适合紧急回退。此修复没有数据库 migration，
无需回滚 `data/v2/`；回滚和重启时必须继续保留现有 `VIDA_PROFILE_MASTER_KEY`。

回滚后按根目录 `AGENTS.md` 的方式激活虚拟环境并重启后端，再运行完整后端测试。若已经存在失败的
Episode，可在确认旧行为符合预期后手动重试；不要删除 Episode 数据来代替代码回滚。
