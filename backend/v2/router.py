...
from .api.episodes import create_dashboard_router, create_episode_router
from .api.events import create_event_router
...
def create_v2_router(runtime: V2Runtime) -> APIRouter:
    router = APIRouter()
    router.include_router(create_event_router(runtime))
    router.include_router(create_dashboard_router(runtime))
